...
/**
 *
 * Validate that a param string and throw error if it is not valid
 * @param resourceSchema
 * @returns
 */
export const RequestParamValidator =
  (resourceSchema: ZodEffects<AnyZodObject> | AnyZodObject) =>
  async (req: Request, res: Response, next: NextFunction) => {
    // throws an error if not valid
    try {
      const value = await resourceSchema.parseAsync(req.params);
      req.body.validatedParamData = value;
      next();
    } catch (error: any) {
      const response = {
        status: false,
...
const errorCleaner = (errors: any) => {
  console.error(`zod error in schema ${JSON.stringify(errors.issues)}`);
  let message;
